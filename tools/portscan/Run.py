...
def WritePorts(ip, subdomain, info_dict):
    ...
    if not SrcPorts.query.filter(SrcPorts.subdomain_ip == ip).count():
        for info in info_dict:
            sql = SrcPorts(subdomain_ip=ip, subdomain=subdomain, port=info_dict[info]['port'],
                           service=info_dict[info]['name'], product=info_dict[info]['product'],
